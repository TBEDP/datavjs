...
;(function (name, definition) {
    ...
    Pie.prototype.render = function () {
        ...
        var mouseClick = function () {
            ...
            var ro = (groups[index].startAngle + groups[index].endAngle) * 90 / Math.PI;
            var a = 0.5 * ((groups[index].startAngle + groups[index].endAngle) - Math.PI);
            var center = {
                x: ((conf.width - that.xOffset) / 2 + that.xOffset),
                y: conf.height / 2
            };
            var namePos = {
                x: conf.protrude * Math.cos(a),
                y: conf.protrude * Math.sin(a)
            };
            var radius = {
                x: conf.radius * Math.cos(a) + namePos.x,
                y: conf.radius * Math.sin(a) + namePos.y
            };
            var linePos = {
                x: 9 * Math.cos(a),
                y: 9 * Math.sin(a)
        ...
        for (i = 0; i <= groups.length - 1; i++) {
            ...
            tips = that.groups[i].nameTag + ": " + Math.round(groups[i].value) + " " + (groups[i].value * 100 / this.sum).toFixed(2) + "%";
            ...
            var nameTag = that.canvas.text().attr("font", "18px Verdana").attr("text", that.groups[i].nameTag).translate(((conf.width - that.xOffset) / 2 + that.xOffset) + nameX, conf.height / 2 + nameY).rotate(ro); //transform(nameStr);
    ...
    Pie.prototype.legend = function () {
        ...
        rectBn.forEach(function (d, i) {
            ...
            d.click(function () {
                var ro = (groups[i].startAngle + groups[i].endAngle) * 90 / Math.PI;
                var a = 0.5 * ((groups[i].startAngle + groups[i].endAngle) - Math.PI);
                var center = {
                    x: ((conf.width - that.xOffset) / 2 + that.xOffset),
                    y: conf.height / 2
                };

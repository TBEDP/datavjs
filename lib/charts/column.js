...
;(function (name, definition) {
  ...
  Column.prototype.drawAxis = function () {
    ...
    var ticks;
    // X轴
    ticks = this.x.ticks(conf.tickNumber);
    for (j = 1; j < ticks.length; j++) {
      tickText.push(paper.text(this.x(ticks[j]), axis.down + 14, conf.xAxisData[ticks[j] - 1]).rotate(0, this.x(ticks[j]), axis.up));
      axisLines.push(paper.path("M" + this.x(ticks[j]) + "," + axis.down + "L" + this.x(ticks[j]) + "," + (axis.down + 5)));
    }
    tickText.attr({
      "fill": "#878791",
      "fill-opacity": 0.7,
    ...
    //Y轴
    ticks = this.value.ticks(conf.tickNumber);
  ...
  Column.prototype.drawDiagram = function () {
    ...
    var barWidth = (axis.right - axis.left - (maxLength - 1) * conf.gap) / maxLength / _.keys(this.columns).length;

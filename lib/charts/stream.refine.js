...
;(function (name, definition) {
  ...
  var StreamChart = require('StreamChart');
  ...
  StreamComponent.prototype.init = function () {
    ...
    that.on('changeLevelTo', function (level) {
      that.defaults.moreConfig.level = level;
      getBack(that.defaults.moreConfig.level);
    });
    that.on('back', function () {
      that.defaults.moreConfig.level = that.defaults.moreConfig.level - 1;
      getBack(that.defaults.moreConfig.level);
    });
    that.chart.on('enter', function () {
      that.axis.showTab();
      that.tip.show();
    ...
    }).on('move', function (pre, cur, xIdx) {
      if (pre !== undefined) {
        that.legend.lowlight(pre);
      }
      that.legend.highlight(cur);
      that.tip.setContent(cur);
      //axis pop bubble
      var lineX = that.defaults.chartWidth * xIdx / (that.chart.source[0].length - 1);
      that.axis.refreshTab(that.date[xIdx + that.timeRange[0]], lineX + that.defaults.percentageWidth);
    });
  };
  ...
  StreamComponent.prototype.layout = function () {
    ...
    // 创建DOM节点
    this.streamBox = $("<div></div>");
    this.legendBox = $("<div></div>");
    this.axisBox = $("<div></div>");
    this.naviBox = $("<div></div>");
    this.percentageBox = $("<div></div>");
    this.container = $("<div></div>");
    this.rightBox = $("<div></div>");
    // cover can block stream paper when animating to prevent some default mouse event
    this.coverBox = $("<div></div>");
    // 插入DOM
  ...
  StreamComponent.prototype.draw = function () {
    var conf = this.defaults;
    //chart and paper
    this.chart = new StreamChart(this.streamBox, {"width": conf.chartWidth, "height": conf.chartHeight});
    this.chart.setOptions(this.defaults);
    this.legend = this.own(new Legend.Legend(this.legendBox));
    this.legend.setOptions({
      "colorFunc": this.chart.getColor()
    });

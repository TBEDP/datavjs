...
(function (name, def) {
  ...
  function drawGridConsume (r, x, y, w, h, wv, hv, color, options) {
    color = color || "#999999";
    var path = [];
    var rowHeight = h / hv,
      columnWidth = w / wv;
  ...
  LevelChart.prototype.render = function (data, options) {
    ...
    var globalBase = options && options.globalBase || [8, 17, 50, 17, 8];
    
    if (!nobase && globalMax < 50) {
      globalMax = 50;
    }

    drawGridConsume(r, 0, topGutter + 0.5, width, height - topGutter - bottomGutter, 0, 6, '#cccbbb', options);

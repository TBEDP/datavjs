...
var TifBarLevel = function (config) {
  ...
  data.forEach(function (d, i) {
    var rect = paper.rect(margin.left + barH * i, margin.top + w - wArr[i], barH, wArr[i])
      .attr({
        "r": 3,
        "cursor": "pointer",
  ...
  ratioArr.forEach(function (d, i) {
    ...
    var text = paper.text(
      margin.top + barH * (i + 0.5),
      margin.top - 20 + w - wArr[i],
  ...
  data.forEach(function (d, i) {
    ...
    title.css({
      'position': 'absolute',
      'left': margin.top + barH * (i + 0.5),
      'top': margin.left + h + 10,
      'text-align': 'right'
    });
    titles.push(title);
  });

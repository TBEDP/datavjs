...
;(function (name, definition) {
   ...
   */
   var Line = DataV.extend(DataV.Chart, {
    initialize: function (node, options) {
      ...
      //Properties
      this.defaults.margin = [10, 50, 50, 50];
  ...
  Line.prototype.setSource = function (source, map) {
    ...
    if (!map.x){
      xAxesData = source[0];
      linesData = source.slice(1);
      maxLength = xAxesData.data.length;
      _.each(source, function (obj) {
        if (obj.name != map.x){
          y_maxList.push( Math.max.apply(null, obj.data) );
          y_minList.push( Math.min.apply(null, obj.data) );
        }
        maxLength = Math.max(maxLength,obj.data.length);
      });
    }
    else{
      maxLength = source[0].data.length;
      _.each(source, function (obj) {
        maxLength = Math.max(maxLength,obj.data.length);
        if (obj.name == map.x){
          xAxesData = obj;
    ...
    this.xAxesData = xAxesData;
    this.linesData = linesData;
    this.maxLength = maxLength;
    this.maxX = Math.max.apply(null, xAxesData.data);
    this.minX = Math.min.apply(null, xAxesData.data);
    this.maxY = Math.max.apply(null, y_maxList);
    this.minY = Math.min.apply(null, y_minList);
    this.chosen = false;
    this.chosenNum = -1;
  };
   ...
   * 绘制X轴
   */
  Line.prototype.setXaxis = function (xDatas) {
    var conf = this.defaults;
    ...
    if (conf.hasXAxis) {
      ...
      for (var i = 0; i < tick; i++) {
        ...
        if (i < l) {
          var tmpXVal = minX + i * d;
          if (tmpXVal  < 0.001){
            tmpXVal = tmpXVal.toExponential(2);
          }
          else {
            tmpXVal = tmpXVal.toFixed(2);
          }
          // 绘制横坐标上的文字
          var text = canvas.text((startX + i * tickStep), startY + xAxisPadding, tmpXVal );
          if (textLean) {
  ...
  Line.prototype.setYaxis = function () {
    ...
    if (conf.hasYAxis) {
     ...
     // }
      ...
      for (var i = 0; i < tick; i++) {
       var tmpYVal = minY + i * d;
          if (tmpYVal  < 0.001){
            tmpYVal = tmpYVal.toExponential(2);
          }
          else {
            tmpYVal = tmpYVal.toFixed(2);
          }
        yGrid.push(canvas.path("M" + startX + "," + (startY - i * tickStep) + "L" + (startX + tickLength) + "," + (startY - i * tickStep)));
        yAxisText.push(canvas.text(startX - yAxisPadding, (startY - i * tickStep), tmpYVal));
      }
  ...
  Line.prototype.render = function () {
    ...
    linesData.forEach(function (d, i) {
      ...
      for (var j = 0, l = nodeData.length; j < l; j++) {
        var x = x0 + ((xAxesData.data[j] - minX) * xMatchNum);//xTick * (j);
        var y = y0 - ((nodeData[j] - minY) * yMatchNum);
        ...
        if (nodeMode) {
          ...
          thisNode.data('data', "(" + xAxesData.data[j] + ", " + nodeData[j] + ")" );
          nodes.push(thisNode);
        }
      }

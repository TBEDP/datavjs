...
;(function (name, definition) {
    ...
    Bubble.prototype.generatePaths = function (time) {
        ...
        for (var i = 0, l = keys.length; i < l; i++) {
            x0 = this.interpolateData(time, timeKeys, this.getKeyData(this.xDimen, keys[i]));
            y0 = this.interpolateData(time, timeKeys, this.getKeyData(this.yDimen, keys[i]));
            r0 = this.interpolateData(time, timeKeys, this.getKeyData(this.sizeDimen, keys[i]));
        ...
        var label = foreCanvas.text(20, m[1] + realHeight + 15, this.times[time.toFixed(0)]);
        ...
        for (var i = 0, l = dots.length; i < l; i++) {
            var dot = dots[i],
                x = this.x[this.xDimen](dot.x0) - m[0],

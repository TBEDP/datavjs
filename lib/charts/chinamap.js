...
;(function (name, definition) {
     ...
     * Options:
     *
     *   - `width` 数字，图片宽度，默认为600，表示图片高600px
     *   - `height` 数字，图片高度，默认为500
     *   - `geoDataPath` 字符串，地图数据的文件路径，如"../../lib/charts/data/chinaMap/"
     *   - `mapId` 字符串，地图区域的Id,用于载入相应的地图文件。全国时，mapId为"0";新疆为"65"。其他各省Id可参考this.geoData.areaIdIndex。
     ...
     *   - `customEvent` 函数对象，其中有4个自定义函数。`areaHoverIn` 函数，表示鼠标移上某个区域的事件响应，默认为空函数; `areaHoverOut` 函数，表示鼠标移出某个区域的事件响应，默认为空函数; `areaClick` 函数，表示点击某个区域的事件响应，默认为空函数; `wordHoverIn` 函数，表示鼠标移上某个地名文字时的事件响应; `wordHoverOut` 函数，表示鼠标移出某个地名文字时的事件响应; `wordClick` 函数，表示点击某个地名文字的事件响应，默认为空函数;这些函数可以在创建对象或setOption()时一起设置。
    ...
    var Chinamap = DataV.extend(DataV.Chart, {
        initialize: function (node, options) {
            ...
            */
            ...
            this.sourceData = {};
            this.geoData = {};
            this.geoData.areaIdIndex = {//区域与编号的对应关系。
                '全国': 0,
                '新疆': 65,
                '西藏': 54,
                '内蒙古': 15,
            ...
            this.geoData.areaBoxes = {//各区域的矩形边界
            ...
            this.geoData.provinceIndex = {//省的索引，包含经纬度和全名信息
    ...
    /**
     * 获取区域颜色。内部函数。用户设置colorModel和colors来设置区域颜色。输入为地图区域对象，内部对象，包含某个地图区域的信息。内部的实现流程是：如果颜色生成函数不存在，则根据colorModel生成颜色生成函数。"discrete"时颜色在colors中从头至尾循环选取，"gradient"时依照colors中颜色的渐变插值计算。离散色时颜色有区域的分类决定。渐变色时颜色由区域的值决定（最小值和最大值间进行插值）。
     */
     ...
     * ```
     * chinamap.setSource({//示例一，区域有值，全国各省人口数据，适合用渐变色显示。
            '北京': 19612368,
            '天津': 12938224,
            '河北': 71854202,
            '山西': 35712111,
        ...
        chinamap.setSource({//示例二，区域分成各个类，适合用渐变色显示。
            '北京': '华北',
            '天津': '华北',
            '河北': '华北',
            '山西': '华北',
        ...
        var render = function (areaId, json) {
            ...
            if (typeof mapCache[areaId] === 'undefined') {//no cache
                ...
                json.features.forEach(function (d, i) {
                    ...
                    .data("info", d)
                    .click(function () {
                      map.fire("areaClick", this);
                      customEvent.areaClick.call(this);
                    })
                    .mouseover(function () {
                      map.fire("areaHoverIn", this);
                      customEvent.areaHoverIn.call(this);
                    })
                    .mouseout(function () {
                      map.fire("areaHoverOut", this);
                      customEvent.areaHoverOut.call(this);
                    });
                ...
                json.features.forEach(function (d, i) {
                    ...
                    .click(function () {
                      ...
                      customEvent.wordClick.call(this);
                    }).mouseover(function () {
                      map.fire("wordHoverIn", this);
                      customEvent.wordHoverIn.call(this);
                    }).mouseout(function () {
                      map.fire("wordHoverOut", this);
                      customEvent.wordHoverOut.call(this);
                    });
                    if (!conf.showWords) {
                        word.hide();
                    }

...
;(function (name, definition) {
  ...
  Stream.prototype.remapSource = function () {
    ...
    this.rowCount = _.max(_.map(_.values(grouped), function (i) {
      return i.length;
    }));
    var keys = _.keys(grouped);
    this.columnCount = keys.length;
    var remap = [];
    for (var j = 0; j < this.columnCount; j++) {
      var sum = DataV.sum(this.grouped[keys[j]], this.mapping.value);
      for (var i = 0; i < this.rowCount; i++) {
        remap[i] = remap[i] || [];
        remap[i][j] = {};
        remap[i][j].x = j;
        var rate = grouped[keys[j]][i][this.mapping.value] / sum;
        remap[i][j].y = rate;
        grouped[keys[j]][i].rate = rate;
      }
    }
    return remap;
  };

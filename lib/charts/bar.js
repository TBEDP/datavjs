...
;(function (name, definition) {
  ...
  Bar.prototype.drawDiagram = function () {
    ...
    var mouseOutBar = function (event) {
        ...
        tagSet.stop().animate({"opacity": 0}, 0, function () {
            tagSet.remove();
        });
        currentSet.forEach(function (set) {
            set.animate({"fill-opacity": 1}, 0);
        });
    };
    ...
    values.forEach(function (bar, index) {
      ...
      bar.forEach(function (row, i) {
        ...
        if(index == 0) {
            var backRect = paper.rect(axis.left, y - barWidth * (index + 1) - conf.gap / 2 - barWidth + backRectWidth / 2, 
                axis.right - axis.left, backRectWidth).attr({
                "fill": "#ffffff",
                "fill-opacity": 0,
                "stroke": "none"
            });
        } else if( index == values.length - 1) {
            var backRect = paper.rect(axis.left, y - barWidth * (index + 1) - conf.gap / 2 - backRectWidth / 2, 
                axis.right - axis.left, backRectWidth).attr({
                "fill": "#ffffff",
                "fill-opacity": 0,
                "stroke": "none"
            ...
            var backRect = paper.rect(axis.left, y - barWidth * (index + 1) - conf.gap / 2, axis.right - axis.left, barWidth).attr({
                "fill": "#ffffff",
                "fill-opacity": 0,
                "stroke": "none"
            });
        ...
        backRect.mouseover(mouseOverBar);
        backRect.mouseout(mouseOutBar);

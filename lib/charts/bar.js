...
;(function (name, definition) {
  ...
  Bar.prototype.drawAxis = function () {
    ...
    for (j = 0; j < ticks.length; j++) {
      ...
      // 修复x轴单位对于柱位置的偏移
      var y = this.y(ticks[j]) - this.barCount * Math.floor(conf.barWidth) / 2;
  ...
  Bar.prototype.drawDiagram = function () {
    ...
    var mouseOutBar = function (event) {
        var barIndex = this.data('rect').data('bar');
        var yIndex = this.data('rect').data('index');
        ...
        currentSet.forEach(function (set) {
            set.animate({"fill-opacity": 1}, 100);
        });
    };
    
    var mouseMoveBack = function (event) {
        var mouseX = event.x;
        var mouseY = event.y;
        var step = that.y(0) - that.y(1);
        var seq = Math.floor((axis.down - mouseY) / step);
        var y = that.y(seq);
        var barIndex = Math.floor((y - mouseY) / barWidth);
        if (barIndex < 0) {
            barIndex = 0;
        } else if (barIndex > barSet.length - 1) {
            barIndex = barSet.length - 1;
        }
        ...
        hovered.forEach(function (item, barIndex) {
            ...
            var valueLabel = formatValue('' + values[barIndex][seq][dim.value.index]);
        ...
        var formatLabel = conf.formatLabel || that.formatLabel;
        var yLabel = formatLabel('' + values[barIndex][seq][dim.x.index]);
    ...
    var mouseOutBack = function (event) {
        tagSet.remove();
        barSet.forEach(function (set, barIndex) {
            set.forEach(function (bar, index) {
                bar.attr({"fill-opacity": 1});
            });
        });
    ...
    values.forEach(function (bar, index) {
      ...
      bar.forEach(function (row, i) {
        ...
        var y = that.y(i);
        var rect = paper.rect(axis.left, y - barWidth * (index + 1), height - axis.left, barWidth).attr({
        ...
        //draw back ground rect
        //var backRectWidth = (that.y(i) - that.y(i + 1)  - values.length * barWidth)/2 + barWidth;
        //var backRect;
        //if(index === 0) {
        //    backRect = paper.rect(axis.left, y - barWidth * (index + 1) - conf.gap / 2 - barWidth + backRectWidth / 2, 
        //        axis.right - axis.left, backRectWidth);
        //} else if( index === values.length - 1) {
        //    backRect = paper.rect(axis.left, y - barWidth * (index + 1) - conf.gap / 2 - backRectWidth / 2, 
        //        axis.right - axis.left, backRectWidth);
        //} else {
        //    backRect = paper.rect(axis.left, y - barWidth * (index + 1) - conf.gap / 2, axis.right - axis.left, barWidth)
        //}
        //backRect.attr({
        //        "fill": "#ffffff",
        //        "fill-opacity": 0,
        //        "stroke": "none"
        //    });
        //backRect.data('rect', rect);
        //backRect.mouseover(mouseOverBar).mouseout(mouseOutBar);
        //backSet.push(backRect);
        
      });
    });
    
    var backSet = paper.set();
    var backRect = paper.rect(axis.left, axis.up, axis.right - axis.left, 
            axis.down - axis.up).attr({
                "fill": "#000000",
                "fill-opacity": 0,
                "stroke": "none"
            });
    backRect.mousemove(mouseMoveBack).mouseout(mouseOutBack);
    backSet.push(backRect);
  };
